...
/*
  ...
  -In two points in this file we used an std::atomic<bool>. Probably the code would work even without using that an atomic value, because those values are protected by a mutex. I have not tested it
  ...
  -First of all, it seems working (even if I have not made enoght tests), so it can be used for ispiration if multithreading is needed
...
{
    {
        ...
        T * element_to_return = Asynchronous_loader::buffers[(int)current_buffer][current_element_in_buffer];
